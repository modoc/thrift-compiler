...
# Builds thrift executable into target/thriftc.exe
...
if [[ $# < 3 ]]; then
    echo "Arguments missing. Usage: ScriptName <OS> <ARCH> <VERSION>"
    exit 1
fi
...
checkArch()
{
    ...
    if [[ "$OS" == windows || "$OS" == linux ]]; then
        format="$(objdump -f "$1" | grep -oE "(file format|文件格式) .*$" | grep -o "[^ ]*$")"
        echo Format=$format
        if [[ "$OS" == linux ]]; then
            if [[ "$ARCH" == x86_32 ]]; then
...
# Checks the dependencies of the artifact. Artifacts should only depend on system libraries.
...
echo "Building thriftc, OS=$OS ARCH=$ARCH"
...
TARGET_FILE="target/thriftc-$VERSION-$OS-$ARCH.exe"
...
THRIFT_NAME="thrift-$VERSION"
if [[ ! -d $THRIFT_NAME ]];then
    wget http://archive.apache.org/dist/thrift/$VERSION/$THRIFT_NAME.tar.gz
    tar -zxvf $THRIFT_NAME.tar.gz
fi

CONFIGURE_ARGS="$CONFIGURE_ARGS CXXFLAGS='$CXXFLAGS' LDFLAGS='$LDFLAGS'"
CONFIGURE_ARGS="$CONFIGURE_ARGS --prefix=$WORKING_DIR/$THRIFT_NAME/thriftc/thrift --with-nodejs=no"

echo $CONFIGURE_ARGS

PY_PREFIX_TEMP=$PY_PREFIX
PHP_PREFIX_TEMP=$PHP_PREFIX

export PY_PREFIX="$WORKING_DIR/$THRIFT_NAME/thriftc/py"
export PHP_PREFIX="$WORKING_DIR/$THRIFT_NAME/thriftc/php"

cd "$WORKING_DIR"/"$THRIFT_NAME" && ./bootstrap.sh && ./configure $CONFIGURE_ARGS && mkdir thriftc
    make clean && make install &&
    cd "$WORKING_DIR" && mkdir -p target &&
    cp "$THRIFT_NAME/thriftc/thrift/bin/thrift" $TARGET_FILE ||
    exit 1
...
export PY_PREFIX=$PY_PREFIX_TEMP
export PHP_PREFIX=$PHP_PREFIX_TEMP
